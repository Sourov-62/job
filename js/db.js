  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      if (!db.objectStoreNames.contains(STORES.exams)) {
        const s = db.createObjectStore(STORES.exams, { keyPath: "id" });
        s.createIndex("createdAt", "createdAt");
      }
      if (!db.objectStoreNames.contains(STORES.examAttempts)) {
        const s = db.createObjectStore(STORES.examAttempts, { keyPath: "id" });
        s.createIndex("examId", "examId");
        s.createIndex("createdAt", "createdAt");
      }
      if (!db.objectStoreNames.contains(STORES.currentAffairs)) {
        const s = db.createObjectStore(STORES.currentAffairs, { keyPath: "id" });
        s.createIndex("date", "date");
      }
      if (!db.objectStoreNames.contains(STORES.pyq)) {
        const s = db.createObjectStore(STORES.pyq, { keyPath: "id" });
        s.createIndex("examName", "examName");
