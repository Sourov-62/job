...
function rerenderCurrentView() {
  switch (state.currentView.type) {
    ...
    case "challengeHistory": return renderChallengeHistoryPage();
...
function renderSidebar() {
  ...
  const navCh = qs("#navChallengeHistory");
  if (navCh) navCh.classList.toggle("active", state.currentView.type === "challengeHistory");
  const navAr = qs("#navArchive");
  if (navAr) navAr.classList.toggle("active", state.currentView.type === "archive");
  const navBd = qs("#navBadges");
  if (navBd) navBd.classList.toggle("active", state.currentView.type === "badges");
...
if (qs("#navChallengeHistory")) qs("#navChallengeHistory").addEventListener("click", () => navigateChallengeHistory());
if (qs("#navArchive")) qs("#navArchive").addEventListener("click", () => navigateArchive());
if (qs("#navBadges")) qs("#navBadges").addEventListener("click", () => navigateBadges());
...
function navigateChallengeHistory() {
  state.currentView = { type: "challengeHistory" };
  renderSidebar();
  renderChallengeHistoryPage();
}

/* এই পেজটা আগে index.html-এ স্ট্যাটিক HTML হিসেবে ছিল, কিন্তু অন্য যেকোনো ভিউ রেন্ডার হওয়ার
   সাথে সাথেই #mainView-এর innerHTML পুরোপুরি বদলে যায় বলে সেই স্ট্যাটিক অংশটা প্রথম রেন্ডারেই
   হারিয়ে যেত এবং সাইডবারের "চ্যালেঞ্জ হিস্টোরি" বাটনে ক্লিক করলে কিছুই দেখা যেত না।
   এখন অন্য সব ভিউয়ের মতোই এটাও ডাইনামিকভাবে রেন্ডার হয়। */
function renderChallengeHistoryPage() {
  const main = qs("#mainView");
  main.innerHTML = `
    <div class="page-header">
      <div>
        <h1 class="page-title" id="page-challenge-title">🔥 ডেইলি চ্যালেঞ্জ ও স্ট্রিক হিস্টোরি</h1>
      </div>
      <span class="streak-badge" id="page-streak-count">0 Days Streak 🔥</span>
    </div>
    <div class="challenge-history-grid">
      <div class="history-card">
        <h3 id="page-today-title">📅 আজকের লক্ষ্য</h3>
        <div id="page-challenge-text"></div>
        <button id="page-challenge-btn" class="btn btn-accent" style="width:100%;margin-top:15px;padding:12px;font-weight:bold;border-radius:8px;" onclick="DailyChallenge.submitToday()"></button>
      </div>
      <div class="history-card">
        <h3 id="page-history-title">📜 দৈনিক হিস্টোরি রেকর্ড</h3>
        <div id="page-challenge-history-list"></div>
      </div>
    </div>
  `;
...
/* ===================== Current Affairs ও PYQ আর্কাইভ ===================== */
function navigateArchive(tab = "affairs") {
  state.currentView = { type: "archive", tab };
  renderSidebar();
  renderArchiveView();
}
function renderArchiveView() {
  const main = qs("#mainView");
  const tab = state.currentView.tab || "affairs";
  main.innerHTML = `
    <div class="page-header">
      <div><h1 class="page-title">${t("nav_archive")}</h1></div>
    </div>
    <div class="tabs" id="archiveTabs">
      <button class="tab-btn ${tab === "affairs" ? "active" : ""}" data-tab="affairs">${t("archive_tabAffairs")}</button>
      <button class="tab-btn ${tab === "pyq" ? "active" : ""}" data-tab="pyq">${t("archive_tabPyq")}</button>
    </div>
    <div id="archiveContent"></div>
  `;
  qsa("#archiveTabs .tab-btn").forEach((b) => {
    b.addEventListener("click", () => {
      state.currentView.tab = b.dataset.tab;
      renderArchiveView();
    });
  });
  if (typeof ArchiveView !== "undefined") ArchiveView.render(tab, qs("#archiveContent"));
}

/* ===================== Badge ও লেভেল ===================== */
function navigateBadges() {
  state.currentView = { type: "badges" };
  renderSidebar();
  renderBadgesPage();
}
function renderBadgesPage() {
  ...
  main.innerHTML = `
    <div class="page-header">
      <div><h1 class="page-title">${t("badges_pageTitle")}</h1></div>
    </div>
    <div id="badgesContent"></div>
  `;
  if (typeof Gamification !== "undefined") Gamification.render(qs("#badgesContent"));
}
...
function openMobileMoreSheet() {
  openModal(`
    ...
    <div class="sheet-list">
      ...
      <button class="sheet-list-item" id="sheetGoChallengeHistory">🔥 ${t("nav_challengeHistory")}</button>
      <button class="sheet-list-item" id="sheetGoArchive">🗞️ ${t("nav_archive")}</button>
      <button class="sheet-list-item" id="sheetGoBadges">🏅 ${t("nav_badges")}</button>
  ...
  qs("#sheetGoChallengeHistory").addEventListener("click", () => { closeModal(); navigateChallengeHistory(); });
  qs("#sheetGoArchive").addEventListener("click", () => { closeModal(); navigateArchive(); });
  qs("#sheetGoBadges").addEventListener("click", () => { closeModal(); navigateBadges(); });
...
async function renderDashboard() {
  ...
  main.innerHTML = `
    <div class="page-header">
      ...
      <div style="display:flex;align-items:center;gap:10px;">
        <button class="level-mini-chip-btn" id="dashLevelChip" title="ব্যাজ ও লেভেল দেখুন"></button>
  ...
  qs("#dashLevelChip").addEventListener("click", () => navigateBadges());
  if (typeof Gamification !== "undefined") Gamification.renderMiniChip(qs("#dashLevelChip"));
...
function renderSettingsView() {
  ...
  qs("#btnExport").addEventListener("click", async () => {
    ...
    // currentAffairs ও pyq এন্ট্রির attachments (ছবি/PDF) — প্রতিটা attachment-এর
    // ভেতরের blob-ও একইভাবে base64-এ রূপান্তর করতে হবে, নাহলে JSON.stringify ব্যর্থ হবে।
    for (const key of ["currentAffairs", "pyq"]) {
      for (const item of dump[key] || []) {
        if (Array.isArray(item.attachments)) {
          for (const att of item.attachments) {
            if (att.blob instanceof Blob) {
              att.blobBase64 = await blobToBase64(att.blob);
              delete att.blob;
            }
          }
        }
      ...
      for (const key of ["currentAffairs", "pyq"]) {
        for (const item of dump[key] || []) {
          if (Array.isArray(item.attachments)) {
            for (const att of item.attachments) {
              if (att.blobBase64) {
                att.blob = await base64ToBlob(att.blobBase64, att.mime);
...
// ভাষা টগল বাটনে (EN/বাং) ক্লিক করা মাত্রই — এই মুহূর্তে Daily Challenge বা Exam Center
// মডাল খোলা থাকলে তার ভেতরের লেখাও সাথে সাথে নতুন ভাষায় বদলে যাবে। (challenge history
// পেজের রিফ্রেশ ইতিমধ্যে navigateChallengeHistory() থেকে হয়, তাই এখানে আলাদা করে দরকার নেই।)
qs("#langToggleBtn").addEventListener("click", () => {
  if (typeof DailyChallenge !== "undefined" && DailyChallenge.init) DailyChallenge.init();
  const examOpen = qs("#examModal") && !qs("#examModal").classList.contains("hidden");
  if (examOpen && typeof ExamCenter !== "undefined" && ExamCenter.init) ExamCenter.init();
});
